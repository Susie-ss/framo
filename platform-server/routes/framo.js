...
router.post('/ai/generate', async function(req, res) {
  try {
    ...
    // 清理 markdown 代码块标记
    var html = aiResponse.replace(/^```html\s*/i, '').replace(/\s*```$/i, '').trim();
    // 确保 HTML 完整
    ...
    var tokens = library.tokens || { colorPrimary: '#6698FF', colorSurface: '#FFFFFF', borderRadius: '8px', fontSizeBase: 14 };
    var components = library.components || [];

    // 构造 JSON layout，把 AI 真实 HTML 放入，前端预览模板用 iframe 展示
    var layout = {
      type: 'page',
      prompt: prompt,
      ...
      tokens: tokens,
      componentReferences: [
        { role: 'container', component: library.name || '组件库', reason: '页面主容器' },
        { role: 'content', component: components.slice(0, 3).join(', ') || '表单/按钮', reason: '核心交互内容' },
        { role: 'action', component: 'Button 按钮', reason: '用户操作入口' }
      ],
      layout: [{
        type: 'container',
        props: { title: prompt },
        children: [
          { type: 'stats', items: [
            { label: '生成耗时', value: '< 30s', delta: 'AI 实时' },
            { label: '引用组件', value: String(components.length || 8), delta: '组件库' },
            { label: '主色', value: tokens.colorPrimary, delta: 'Token 规范' }
          ]},
    ...
    // 兜底：使用库信息构造布局，不含 mock 数据
    try {
      var fallbackAdvanced = await advancedFramo();
      var fallbackLib = fallbackAdvanced.libraries[0];
      var tokens = fallbackLib.tokens || { colorPrimary: '#6698FF', colorSurface: '#FFFFFF', borderRadius: '8px', fontSizeBase: 14 };
      var components = fallbackLib.components || [];
      res.json({
        ok: true,
        promptTemplate: { libraryId: fallbackLib.id, rules: [] },
        result: {
          type: 'page', prompt: prompt, libraryId: fallbackLib.id, tokens: tokens,
          componentReferences: [
            { role: 'container', component: fallbackLib.name || '组件库', reason: '页面主容器' },
            { role: 'content', component: components.slice(0, 3).join(', ') || '表单组件', reason: '核心交互内容' }
          ],
          layout: [{
            type: 'container', props: { title: prompt },
            children: [
              { type: 'stats', items: [
                { label: '生成状态', value: 'AI 不可用', delta: err.message.slice(0, 20) },
                { label: '组件库', value: fallbackLib.name, delta: '' },
                { label: '主色', value: tokens.colorPrimary, delta: '' }
              ]},
              { type: 'ai-frame', content: '<div style="padding:40px;text-align:center;color:#64748B;font-size:14px">AI 生成暂时不可用，请稍后重试。</div>' }
            ]
          }],
          html: ''
        }
